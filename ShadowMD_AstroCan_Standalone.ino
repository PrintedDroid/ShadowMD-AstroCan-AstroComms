...
String PS3ControllerFootMac = "XX";
String PS3ControllerDomeMAC = "XX";
String PS3ControllerBackupFootMac = "XX";
String PS3ControllerBackupDomeMAC = "XX";
...
String macBytesToString(byte* mac)
{
    // mac[] is LSB-first (like disc_bdaddr), output MSB-first (like standard MAC format)
    // Uses char buffer to avoid heap fragmentation from repeated String concatenation
    char buf[18]; // "XX:XX:XX:XX:XX:XX" + null
    for (int i = 0; i < 6; i++)
    {
        byte b = mac[5 - i];
        buf[i * 3]     = "0123456789ABCDEF"[b >> 4];
        buf[i * 3 + 1] = "0123456789ABCDEF"[b & 0x0F];
        if (i < 5) buf[i * 3 + 2] = ':';
    }
    buf[17] = '\0';
    return String(buf);
...
void parseMacString(String macStr, byte* mac)
{
    // Input is MSB-first string "XX:XX:XX:XX:XX:XX", store as LSB-first (like disc_bdaddr)
    for (int i = 0; i < 6; i++)
    {
        mac[5 - i] = (byte)strtol(macStr.substring(i * 3, i * 3 + 2).c_str(), NULL, 16);
    }
}
...
void setupMacFilters()
{
    ...
    // Foot filter
    if (PS3ControllerFootMac != "XX")
    {
        parseMacString(PS3ControllerFootMac, mac);
        hasBackup = (PS3ControllerBackupFootMac != "XX");
        if (hasBackup)
        {
    ...
    // Dome filter
    if (PS3ControllerDomeMAC != "XX")
    {
        parseMacString(PS3ControllerDomeMAC, mac);
        hasBackup = (PS3ControllerBackupDomeMAC != "XX");
        if (hasBackup)
        {
...
void loadMacsFromEEPROM()
{
    if (!isEEPROMInitialized())
    {
        ...
        PS3ControllerFootMac = "XX";
        PS3ControllerDomeMAC = "XX";
        PS3ControllerBackupFootMac = "XX";
        PS3ControllerBackupDomeMAC = "XX";
    ...
    PS3ControllerFootMac = isMacSlotEmpty(MAC_SLOT_FOOT_PRIMARY) ? "XX" : macBytesToString(mac);

    readMacFromEEPROM(MAC_SLOT_DOME_PRIMARY, mac);
    PS3ControllerDomeMAC = isMacSlotEmpty(MAC_SLOT_DOME_PRIMARY) ? "XX" : macBytesToString(mac);

    readMacFromEEPROM(MAC_SLOT_FOOT_BACKUP, mac);
    PS3ControllerBackupFootMac = isMacSlotEmpty(MAC_SLOT_FOOT_BACKUP) ? "XX" : macBytesToString(mac);

    readMacFromEEPROM(MAC_SLOT_DOME_BACKUP, mac);
    PS3ControllerBackupDomeMAC = isMacSlotEmpty(MAC_SLOT_DOME_BACKUP) ? "XX" : macBytesToString(mac);
...
void checkPairingUSB()
{
    ...
    String mac = macBytesToString(Btd.disc_bdaddr);
...
void handlePairingConnection(String btAddress, PS3BT* controller, boolean isDome)
{
    byte mac[6];
...
{
    // Uses char buffer to avoid heap fragmentation from repeated String concatenation
    return macBytesToString(Btd.disc_bdaddr);
}
